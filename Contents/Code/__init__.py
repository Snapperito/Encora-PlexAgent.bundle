...
def clean_html_description(html_description):
    try:
        if not isinstance(html_description, basestring):
            return ''
        text = re.sub(r'</p>', '\n', html_description)
        text = re.sub(r'<[^>]+>', '', text)
        text = text.replace('&#039;', "'")
        ...
        Log('[Encora] clean_html_description() error: {}'.format(e))
        return ''
...
def Update(metadata, media, lang, force, movie):
    ...
    try:
        ...
        Log('[Encora] Movie mode - json_recording_details - Loaded recording details from: "{}"'.format(ENCORA_API_RECORDING_INFO.format(recording_id)))
...
class EncoraAgent(Agent.Movies):
  name, primary_provider, fallback_agent, contributes_to, accepts_from, languages = 'Encora', True, ['com.plexapp.agents.xbmcnfo'], None, ['com.plexapp.agents.xbmcnfo', 'com.plexapp.agents.localmedia'], [Locale.Language.NoLanguage]
